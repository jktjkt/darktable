...
void dt_control_local_set_cache_images()
{
  dt_job_t j;
  dt_control_local_cache_images_job_init(&j);
  j.user_data=(void *)1;
  dt_control_add_job(darktable.control, &j);
  return;
}

void dt_control_local_reset_cache_images()
{
  dt_job_t j;
  dt_control_local_cache_images_job_init(&j);
  j.user_data=(void *)0;
...
void dt_control_local_cache_images_job_init(dt_job_t *job)
{
  dt_control_job_init(job, "local cache images");
  job->execute = &dt_control_local_cache_images_job_run;
  dt_control_image_enumerator_t *t = (dt_control_image_enumerator_t *)job->param;
  dt_control_image_enumerator_job_selected_init(t);
}
...
int32_t dt_control_local_cache_images_job_run(dt_job_t *job)
{
  long int imgid = -1;
  dt_control_image_enumerator_t *t1 = (dt_control_image_enumerator_t *)job->param;
  GList *t = t1->index;
  while(t)
  {
    ...
    if ((long int)job->user_data == 1)
      dt_image_local_cache_set(imgid);
    else
      dt_image_local_cache_reset(imgid);
    t = g_list_delete_link(t, t);
  }
  return 0;
}
