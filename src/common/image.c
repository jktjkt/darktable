...
static void _image_cache_full_path(const int imgid, char *pathname, int len);
...
void dt_image_full_path(const int imgid, char *pathname, int len, gboolean *from_cache)
{
  ...
  if (*from_cache && !g_file_test(pathname, G_FILE_TEST_IS_REGULAR))
  {
    _image_cache_full_path(imgid, pathname, len);
    *from_cache = TRUE;
  }
  else
    *from_cache = FALSE;
